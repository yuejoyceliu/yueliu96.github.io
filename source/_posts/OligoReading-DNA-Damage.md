...
6. Descriptions of the cell's response to such lesions: How are they repaird, and with what efficiency and accuracy? Are some lesions more significant than outhers (i.e., less reparable)?
...
Nucleobase radicals are the major intermediates generated by the direct (e.g., $dA^{\cdot +}$) and indirect (e.g., $dA^{\cdot}$) effects of $\gamma$-radiolysis.

The deprotonation of $dA^{\cdot +}$ is a key intermediat involve in DNA electron transfer, a process by which radical cations (holes) migrate through DNA. The deprotonation of $dA^{\cdot +}$ generates $dA^\cdot$, which attenuates hole transfer. $dA^\cdot$ oxidizeds dG, likely via a proton coupled electron transfer in which the first step is endothermic (-0.16V) but the overall reaction is thermodynamically favorable (0.13V) and has been observed in similar systems in DNA

# [Mechanism for Radical Cation Transport in Duplex DNA Oligonucleotides](https://pubs.acs.org/doi/abs/10.1021/ja0378254)

![G radical](https://raw.githubusercontent.com/yueliu96/blog_images/master/dna%20Gradical.jpeg)

DNA oligomers have a covalently linked anthraquinone group (AQ) and contaning $[A_nGG]_m$ or $[T_nGG]_m$ segments. Irradiation with UV light that is absorbed by the AQ casues injection of a radical cation into the DNA. The radical cation migrates through the DNA, casuing chemical reaction, primarily at GG steps, that leads to strand cleavage after piperidine treatment. Radical cation hopping (form one site to adjacent sites) in these duplex DNA oligomers is a process that occurs on a microsecond time scale. The hopping rate depends on the number and kind of bases that separate the GG steps. It is fastest when the separating bases on the GG-containing strand are A and somewhat slower when they are T but reaches a limit when there are four or more bases separating the GG steps.



# [Oxidation of DNA: Damage to Nucleobases](https://pubs.acs.org/doi/abs/10.1021/ar900175a)

![duplex break](https://raw.githubusercontent.com/yueliu96/blog_images/master/dna%20duplex%20break.jpeg)

The losss of an electron (ionization) from DNA generates an electron "hole" located most often on its nucleobsaes, that migrates reversibly through duplex dNA by hopping until it is trapped in an ireversible chemical reaction that damages nucleobases. In normal DNA, comprising the four common DNA nucleobases G, C, A and T, reaction occurs most commonly  at a guanine. Both electronic and steric effects control the outcome of this process. It the DNA oligomer does not contain a suitable guanine, then reaction of the radical cation occurs at the thymine of a TT step, primarily by a tandem process.